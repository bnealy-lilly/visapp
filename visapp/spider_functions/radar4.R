bigradar=function (df, axistype = 0, seg = 4, pty = 16, pcol = 1:8, plty = 1:6, 
                   ...
                   palcex = NULL, ...) 
{
  ...
  plot(c(-1,1), c(-1.1, 1.2), type = "n", frame.plot = FALSE, 
  ...
  if (!is.null(vlabels)) 
    VLABELS <- vlabels
  if (is.null(vlcex)) 
    text(xx * 1.1, yy * 1.1, VLABELS)
  else text(xx * 1.1, yy * 1.1, VLABELS, cex = vlcex)
  series <- length(df[[1]])
  SX <- series - 2
  if (length(pty) < SX) {
    ptys <- rep(pty, SX)
  ...
  for (i in 3:series) {
    ...
    if (sum(!is.na(df[i, ])) < 3) {
      ...
      if (is.null(pdensities)) {
        polygon(xxs, yys, lty = pltys[i - 2], lwd = plwds[i -2], 
