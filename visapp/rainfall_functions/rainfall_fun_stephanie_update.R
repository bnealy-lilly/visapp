## Stephanie Strakbein
## Rainfall Animaiton Function

## Required inputs:
# inputdata, studyname, end.point, trt1, trt1_text, trt1_abr, trt2, trt2_text, trt2_abr, speed
#e.g. end.point = PASI, PPASI, etc.

## Optional inputs:
# plot_title, xlab, ylab, round_override_x, round_override_y, bar_type, clock_type, zoom_type
...
# trt1_text <- input$rainfall_trt1_text
# trt1_abr <- input$rainfall_trt1_abr
# trt2 <- input$rainfall_trt2
# trt2_text <- input$rainfall_trt2_text
# trt2_abr <- input$rainfall_trt2_abr
# # 
# # 
# make_it_rain(rainfall_adam[[1]], input$rainfall_studyname, input$rainfall_endpoint, input$rainfall_xlab, input$rainfall_lab,
#              input$selected_path, input$plot_title, input$rainfall_bar_type, "", "", input$rainfall_clock_type,
...
make_it_rain <- function(inputdata, studyname, end.point, xlab, ylab, outpath, plot_title,
  ...
  #################################################
  ###  Generalized Rainfall Animation Function  ###
  #################################################
  ...
  inputdata <- rename(inputdata, c(usubjid = "subjid")) %>%
    ...
    mutate(trtp = ifelse(trtp == trt1, trt1, ifelse(trtp == trt2, trt2, 'wrong')),
       titrated = end.point,
  ...
  uweek <- unique(inputdata$week)
  uweek <- sort(uweek, decreasing = F)
  # uweek
  uvisit=unique(inputdata$avisitn)
  uvisit <- sort(uvisit, decreasing = F)
  # uvisit
  visit_to_week <- tibble(uvisit, uweek)
  visit_to_week <- mutate(visit_to_week, week_lab=paste("Week ", sprintf("%.0f", uweek), sep=''))
  ...
  for(i in min(inputdata$avisitn):max(inputdata$avisitn)){
    p_i <- ceiling(quantile(inputdata$aval[inputdata$avisitn==i], 0.90))
    percentiles = c(percentiles, p_i)
  }
  ...
  # # position bar chart and other elements
  # rect.x.center <- 12
  # rect.x.width <- 2.25
  # rect.y.center <- 26.25
  # rect.y.height <- 12.7
  ...
  Response = " "
  if(xlab=="") {xlab = paste("Baseline", end.point, "Total Score")}
  if(ylab=="") {ylab = paste(end.point, "Total Score")}
  if(plot_title == "") {
  ...
  ### Eliminate records with "NA" in the input inputdata, usually the number of "NA" is low and won't provide valuable information;
  ### "NA" can come from inputdata creation, for example, if baseline inputdata is missing, then the corresponding % change should be "NA";
  ...
  table(inputdata$grp[inputdata$sday==1])
  ...
  ## only keep inputdata with non-NA value 
  # nadata=inputdata[naindex, ]
  ...
  inputdata.int <- inputdata[!row.has.na, ]
  
  inputdata.int1 <- ddply(inputdata.int, .(subjid), function(x) interpolate(x, "avisitn", "aval", rate=r), 
                     .progress = progbar)
  
  inputdata.int2 <- inputdata.int1 %>%
    filter(!is.na(aval)) 
  range(inputdata.int2$aval)
  
  ## Coerce that interpolated aval is within plausible range
  inputdata.int2 <- mutate(inputdata.int2, aval = ifelse(aval<0, 0, aval)) %>% mutate(aval = ifelse(aval > y_aval_max, y_aval_max, aval))
  ...
  worsening <- mutate(worsening, worse = 1, next_visit = avisitn) %>% rename(c(avisitn = "prev_visit"))
  ...
  interpolated <- mutate(interpolated, next_visit = ceiling(avisitn)) %>% rename(c(worse = "prev_worse"))
  interpolated <- merge(interpolated, worsening, by=c("subjid", "trtp", "next_visit"), all.x = T)
  interpolated <- arrange(interpolated, trtp, subjid, avisitn)
  interpolated[is.na(interpolated$worse),"worse"] <- 0
  interpolated <- select(interpolated, -prev_visit.y) %>% rename(c(prev_visit.x = "prev_visit", worse = "next_worse"))
  
  # worsening2 <- filter(interpolated, next_worse==0, prev_worse==0, aval > base) %>% mutate(diff = aval-base)
  # range(worsening2$pimp)
  # range(worsening2$diff)
  ...
  interpolated <- merge(x=interpolated, y=visit_to_week, by.x = "int_vis", by.y = "uvisit") %>% mutate(week=uweek) %>%  select(-uweek, -int_vis)
  ...
  ### These commented out variables are created above, if this step is removed then they must be perfromed here.
  # uvisit=unique(inputdata$avisitn)
  # uvisit <- sort(uvisit, decreasing = F)
  # uweek=unique(inputdata$week)
  # uweek <- sort(uweek, decreasing = F)
  ...
  makescatter <- function(framedata, xvn, yvn, grpvn="grp", tvn="week", shpvc = "titrated", annodata, trtgrp){
    ...
    if(zoom_type=="zoom"){
      img<-basechart + 
        geom_rect(xmin = (x_min-(minor_unit*.6)), ymin = (y_aval_max-(minor_unit*0.15)), xmax = (x_base_max+(minor_unit*0.15)), ymax = (y_aval_max+(minor_unit*0.15)), fill = "white", colour = "white") +
        geom_rect(ymin = (y_min-(minor_unit*.6)), xmin = (x_base_max-(minor_unit*0.15)), ymax = (y_aval_max+(minor_unit*0.15)), xmax = (x_base_max+(minor_unit*0.15)), fill = "white", colour = "white") +
        ...
        annotate("text", x=(round_cutoff_x+(minor_unit*0.04)), y=(round_cutoff_x), label=paste(end.point,"0",sep=""), colour = group.colors.dark[2], fontface="bold", size = 5, hjust=0) +
        annotate("text", x=(round_cutoff_x+(minor_unit*0.04)), y=((round_cutoff_x*0.5)), label=paste(end.point,"50",sep=""), colour = group.colors.dark[3], fontface="bold", size = 5, hjust=0) +
        annotate("text", x=(round_cutoff_x+(minor_unit*0.04)), y=((round_cutoff_x*0.25)), label=paste(end.point,"75",sep=""), colour = group.colors.dark[4], fontface="bold", size = 5, hjust=0) +
        annotate("text", x=(round_cutoff_x+(minor_unit*0.04)), y=((round_cutoff_x*0.1)), label=paste(end.point,"90",sep=""), colour = group.colors.dark[5], fontface="bold", size = 5, hjust=0) +
        ...
        theme(plot.title = element_text(lineheight=.8,  hjust = 0.5, face = "bold", size=textSizeMajor-2, colour="black"), 
              # theme(plot.title = element_text(lineheight=.8,  hjust = 0.5, size=textSizeMinor, colour=(ifelse(trtgrp==trt1_text, "#49A942", "#63478B"))), 
        ...
        geom_text(data=annodata, aes(x=base, y=aval, label=annotext), fontface="bold", size=rel(7), color = "black", hjust=0, vjust=0, inherit.aes = FALSE )
    } else {
      img<-basechart + 
        ...
        geom_segment(x = 0, y = 0, xend = round_cutoff_x, yend = round_cutoff_x*0.25, colour = grpinfo$col[4]) +   
        geom_segment(x = 0, y = 0, xend = round_cutoff_x, yend = round_cutoff_x*0.1, colour = grpinfo$col[5]) +   
        annotate("text", x=(round_cutoff_x+(minor_unit*0.02)), y=(round_cutoff_x), label=paste(end.point,"0",sep=""), colour = group.colors.dark[2], fontface="bold", size = 5, hjust=0) +
        annotate("text", x=(round_cutoff_x+(minor_unit*0.02)), y=((round_cutoff_x*0.5)), label=paste(end.point,"50",sep=""), colour = group.colors.dark[3], fontface="bold", size = 5, hjust=0) +
        annotate("text", x=(round_cutoff_x+(minor_unit*0.02)), y=((round_cutoff_x*0.25)), label=paste(end.point,"75",sep=""), colour = group.colors.dark[4], fontface="bold", size = 5, hjust=0) +
        annotate("text", x=(round_cutoff_x+(minor_unit*0.02)), y=((round_cutoff_x*0.1)), label=paste(end.point,"90",sep=""), colour = group.colors.dark[5], fontface="bold", size = 5, hjust=0) +
  ...
  ### Plot.Clock Function: creates a circle or bar and colors it in based on the percentage of time consumed thru day.current out of the total day.max
  ...
  if(bar_type == "short") {
    ...
    ### Short Bars
    plot.bar <- function(inputdata, vp){
  ...
  #################################################################
    ...
    ### 2 intergrated functions: interdoplot, sinfig2;
    ...
    ### function interdoplot to create figure with interpolated inputdata and scatter plot
    
    interdoplot <- function(framedata, xvn, yvn, grpvn="grp", tvn="week", shpvc = "titrated", sumdata,titleindex,currvis,maxvis,annodata){
      ...
      ### Need to adjust the font size here, added gp = gpar(fontsize=20), moved the "clock and week" to the middle
      ...
      if(clock_type == "round") {
        ...
        img <- grid.arrange(
          ...
          arrangeGrob(sub2=textGrob(title, gp = gpar(fontsize=textSizeMajor, face="bold"),vjust = 0),ncol=1),
          arrangeGrob(yaxis.label, scatter1, scatter2, makeblank(), ncol=4, widths=c(0.05, 0.465, 0.465, 0.02), nrow=1),
          arrangeGrob(xaxis.label, ncol=1, nrow=1),
          legndBody,
        ...
        ### Line Clock
        img <- grid.arrange(
          arrangeGrob(sub2=textGrob(title, gp = gpar(fontsize=textSizeMajor, face="bold"),vjust = 0.75),ncol=1),
          arrangeGrob(clock, ncol=1),
    ...
    sinfig1 = function(inputdata, visit,sumdata,titleindex,currvis,maxvis, annodata){
      ...
      testimg <- interdoplot(framedata=testdata, xvn="base", yvn="aval", grpvn="grp", shpvc = "titrated",
    ...
    for (i in 1:length(visind)) {
      ...
      if ((visind[i] == floor(visind[i])) & (visind[i] != visind[length(visind)]) & visind[i] !=6) {
        for (j in 1:pause.frame.num) {
          ...
          vp <- viewport(width = 0.17, height = 0.05, x = unit(0.1, "npc"), y = unit(0.56, "npc"), clip="off", just="center",angle = 90)
          b=plot.bar(bar.data, vp)
          print(b)
          
          #### bar plot
          bar.data <- filter(endpoint_freq,avisitn==visind[i], trtp==trt2)
          vp <- viewport(width = 0.17, height = 0.05, x = unit(0.575, "npc"), y = unit(0.56, "npc"), clip="off", just="center",angle = 90)
          c=plot.bar(bar.data, vp=vp)
          print(c) 
      ...
      else if ((visind[i] != visind[length(visind)]) & (visind[i] != 6)) {
        frame.counter <- frame.counter+1
        jpeg(filename=file.path(outpath, figind2[frame.counter]), quality=100,width=1280,height=720,type="cairo")            
        ...
        #### bar plot and annotation for Etanercept
        bar.data <- filter(endpoint_freq, avisitn==visind[i], trtp==trt1)
        vp <- viewport(width = 0.17, height = 0.05, x = unit(0.1, "npc"), y = unit(0.56, "npc"), clip="off", just="center",angle = 90)
        b=plot.bar(bar.data, vp=vp)
        print(b)
        
        #### bar plot and annotation for Ixekizumab 80 mg Q2W
        bar.data <- filter(endpoint_freq, avisitn==visind[i], trtp==trt2)
        vp <- viewport(width = 0.17, height = 0.05, x = unit(0.575, "npc"), y = unit(0.56, "npc"), clip="off", just="center",angle = 90)
        c=plot.bar(bar.data, vp=vp)
        print(c) 
      ...
      else if (visind[i] ==6 | (visind[i] == visind[length(visind)])) {
    ...
    sinfig1 = function(inputdata, visit, sumdata, titleindex, currvis, maxvis, annodata){
      ...
      testdata = subset(interpolated, avisitn==visit)
      bardata = filter(endpoint_freq, avisitn == visit) %>%
