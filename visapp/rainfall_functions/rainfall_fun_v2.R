...
make_it_rain <- function(inputdata, studyname, end.point, xlab, ylab, outpath, plot_title,
  ...
  interdoplot <- function(framedata, bardata=NULL, xvn, yvn, grpvn="grp", tvn="week", 
    ...
    if(bar_type=="long") pctbar <- plot.bar.long(bardata)
  ...
  # VIewport settings for small bar plots
  vp_l <- viewport(width = 0.17, height = 0.05, x = unit(0.1, "npc"), y = unit(0.56, "npc"), clip="off", just="center",angle = 90)
  vp_r <- viewport(width = 0.17, height = 0.05, x = unit(0.575, "npc"), y = unit(0.56, "npc"), clip="off", just="center",angle = 90)
  
  ncores <- detectCores()
  cl <- makeCluster(ncores)
  
  
  foreach(i %in% 1:length(visind)) %dopar% {
    
    paste("Visind is", visind)
    
    if ((visind[i] == floor(visind[i])) & (visind[i] != visind[length(visind)])) {
      if(j %in% 1:pause.frame.num) {
